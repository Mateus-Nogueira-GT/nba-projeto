 ...
 *
 * DUAS telas a mostram: `/como-funciona`, alcançável pelo botão do cabeçalho da
 * Lista e pelo Perfil, e `/metodologia`, o portão de aceite por onde toda conta
 * passa uma vez. Texto duplicado divergiria na primeira vez que alguém
 * corrigisse uma frase só de um lado.
 *
 * SÍNCRONO, e a leitura sai em `lerMetodologia()` logo abaixo. Um componente
 * async ANINHADO suspende dentro de `renderToStaticMarkup`, que é como toda a
 * suíte renderiza tela — só a página de topo pode ser esperada. Então quem lê é
 * a página, numa linha, e o que desenha não lê nada.
 */
export async function lerMetodologia(): Promise<Metodologia> {
...
export type Metodologia = {
  ruleset: Awaited<ReturnType<typeof rulesetAtivo>>
  t: ReturnType<typeof montarTeoria>
  faixasConfianca: Awaited<ReturnType<typeof rulesetAtivo>>['confianca_exibicao']['faixas']
}
    ...
    <div>
      ...
      <Secao titulo="A borda colorida: o nível do apito">
        <p style={{ margin: '0 0 10px' }}>
          ...
          confundir com o nível do jogador. Quanto mais jogos seguidos abaixo da média, mais forte.
        </p>
      ...
      <Secao titulo="As odds">
        ...
        <p style={{ margin: '8px 0 0' }}>
          ...
          como referência. Ela não é uma cotação atual da sua casa. A plataforma é somente leitura:
          não envia apostas nem vincula contas de casas.
        </p>
      </Secao>
      ...
      <Secao titulo="Fire Live — o ao vivo do 1º quarto">
        ...
        <p style={{ margin: 0, fontSize: 13, color: semantico.textoSecundario }}>
          Em pontos,{' '}
          {t.fireLive.presencaTopo.bloqueiaNiveis.map((nv) => NIVEL_JOGADOR[nv].rotulo).join(' e ')}{' '}
      ...
      <Secao titulo="Um exemplo de card">
        ...
        <p style={{ margin: '8px 0 0', fontSize: 13, color: semantico.textoSecundario }}>
          ...
          avatar, e o cruzamento com a OPD sinalizado quando existe. O nome leva às estatísticas do
          jogador.
        </p>
      </Secao>
