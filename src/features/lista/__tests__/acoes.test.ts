...
/**
 * AS SERVER ACTIONS DO AGRUPAMENTO E DAS LENTES — as da Lista do front v2
 * (`features/lista/acoes.ts`), que substituíram `(app)/preferencias/acoes.ts`
 * junto com a home antiga (Tarefa 3). Mesmas invariantes, formulário novo: o
 * botão manda o CAMPO (`ordem`/`lente`) e o `destino` (a URL de onde veio).
 *
 * O caminho inteiro sob teste: FormData → `lerEstadoDaTabela` (validação campo
 * a campo) → `gravarPreferencias` (conta) → `redirect` para a rota REMONTADA. É a fronteira em que um `destino` forjado
...
const formulario = (destino: string, campos: Record<string, string> = {}) => {
...
describe('definirOrdem / definirLente — a escolha vira preferência da conta', () => {
  it('grava a ordem escolhida e redireciona para a rota do destino', async () => {
    const { definirOrdem } = await import('../acoes')
  ...
  it('grava a lente sem varrer a ordem já gravada — nem o recorte da URL', async () => {
    const { definirLente } = await import('../acoes')
  ...
  it('destino forjado não vira redirect para fora do app nem grava valor inventado', async () => {
    const { definirOrdem, definirLente } = await import('../acoes')
    const antes = await preferenciasDoUsuario(banco.db, usuarioId)

    await definirOrdem(formulario('https://evil.com/?ordem=POR_SORTE', { ordem: 'POR_SORTE' }))
    await definirLente(formulario('//evil.com/?lente=RAIO_X', { lente: 'RAIO_X' }))
    await definirOrdem(formulario('javascript:alert(1)', { ordem: 'javascript:alert(1)' }))
