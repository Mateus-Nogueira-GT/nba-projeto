...
export type AtivarAlertasProps = {
  /** Da sessão do servidor (a página passa): entra só na deduplicação local do reenvio. */
  usuarioId: string
...
export function AtivarAlertas({
  ...
  return (
    <section className={estilos.subbloco} aria-labelledby="titulo-alertas-push">
      <p id="titulo-alertas-push" className={estilos.rotulo}>
        Alertas neste aparelho
      </p>

      {estado === 'convite' && (
        <>
          <p className={estilos.textoAlerta}>
      ...
      {(estado === 'pronto' || estado === 'erro') && (
        <>
          ...
          {erro !== null && (
            <p className={estilos.status} role="alert">
              {erro}
            </p>
          )}
          <div className={estilos.acoesLinha}>
        ...
        <>
          ...
          {preferencias !== null && (
            <fieldset className={estilos.grupo}>
              <legend className={estilos.rotulo}>O que você quer receber</legend>
              {CANAIS_ALERTA.map((canal) => (
                <label key={canal} className={estilos.alternarSimples}>
                  <input
                    type="checkbox"
      ...
      {estado === 'indisponivel' && (
        <p className={estilos.status} role="status">
          Os alertas no celular ainda não estão disponíveis neste ambiente.
        </p>
      )}
    </section>
  )
