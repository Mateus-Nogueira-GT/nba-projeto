...
import { gravarConferencia } from '@/app/__tests__/conferencia'
...
/**
 * FUMAÇA DA CONTA DO V2 — herdeira de `telas-05-conta.test.ts`.
 *
 * A suíte antiga foi movida para cá quando a página passou a ser a do v2
 * (`features/conta/{carregar,Blocos}` sobre as MESMAS ações e leituras). As
 * invariantes ficaram; o que mudou foi só como a marcação as expressa — cada
 * adaptação está marcada com "Front v2 (Tarefa 7)".
 *
 ...
 */
// Front v2 (Tarefa 7): o título do bloco é "Assinatura" (a caixa alta é CSS);
// o `id` do título é a âncora estável.
const MARCA = {
  conta: 'id="bloco-Conta"',
  assinatura: 'id="bloco-Assinatura"',
  alertas: 'id="bloco-Alertas"',
  dispositivos: 'id="bloco-Dispositivos"',
} as const

function blocoDeAssinatura(html: string): string {
  const inicio = html.indexOf(MARCA.assinatura)
  const fim = html.indexOf(MARCA.alertas)
...
describe('perfil — a conta da pessoa, não um relatório sobre ela (spec 12/09, §4.3)', () => {
  ...
  it('"Sair" é secundário: contorno, não o azul do CTA (correções UX 19/09)', async () => {
    ...
    // 05 o promoveu por engano. Front v2 (Tarefa 7): a classe é de módulo.
    const { default: Pagina } = await import('@/app/(app)/conta/page')
    const html = renderToStaticMarkup(await Pagina({ searchParams: Promise.resolve({}) }))
    const sair = /<button[^>]*>Sair da conta<\/button>/.exec(html)?.[0] ?? ''
    expect(sair, 'o botão Sair não foi encontrado').not.toBe('')
    expect(sair).toMatch(/class="[^"]*botaoSecundario/)
    expect(sair).not.toMatch(/botaoPrimario|botao-primario/)
  })

  it('sair passa pela ação real que revoga a sessão no banco antes de apagar o cookie', async () => {
    const { readFileSync } = await import('node:fs')
    const pagina = readFileSync('src/app/(app)/conta/page.tsx', 'utf8')
    expect(pagina).toContain("import { sair } from '@/features/publico/acoes'")
    const acoes = readFileSync('src/features/publico/acoes.ts', 'utf8')
    const corpo = acoes.slice(acoes.indexOf('export async function sair'))
    expect(corpo.indexOf('encerrarSessaoPorToken(')).toBeGreaterThan(0)
    expect(corpo.indexOf('limparCookieDeSessao(')).toBeGreaterThan(corpo.indexOf('encerrarSessaoPorToken('))
  })
...
describe('perfil — segurança e dispositivos (spec 12/09, §4.3, Task 5)', () => {
  ...
  it('cada dispositivo tem "encerrar sessão", o aparelho em uso está marcado, e os outros ativos dizem "ativo"', async () => {
    ...
    try {
      ...
      // Front v2 (Tarefa 7): virou o marcador "Ativo" — um só (o inativo, sem
      // sessão viva, não o recebe; o aparelho em uso recebe "Este aparelho").
      expect(html.match(/>Ativo</g) ?? []).toHaveLength(1)
    } finally {
...
describe('perfil — quatro blocos, assinatura e o desktop em duas colunas (spec 12/09, §4.3, Task 6)', () => {
  ...
  function blocoDeAlertas(html: string): string {
    const inicio = html.indexOf(MARCA.alertas)
    const fim = html.indexOf(MARCA.dispositivos)
    expect(inicio).toBeGreaterThanOrEqual(0)
    expect(fim).toBeGreaterThan(inicio)
    return html.slice(inicio, fim)
  }
  ...
  it('sem plano não parece erro: é uma chamada com o botão de assinar, não quatro traços', async () => {
    // Front v2 (Tarefa 7): "sem plano" é o GRÁTIS. Na tela antiga o convite
    // aparecia para qualquer um sem contrato — inclusive um MVP de cortesia,
    // que o v2 mostra, com razão, como "Plano · MVP".
  ...
  it('os quatro blocos, nesta ordem: Conta · Assinatura · Alertas · Dispositivos — e a grade do desktop', async () => {
    const { default: Pagina } = await import('@/app/(app)/conta/page')
    const html = renderToStaticMarkup(await Pagina({ searchParams: Promise.resolve({}) }))
    const ordem = [MARCA.conta, MARCA.assinatura, MARCA.alertas, MARCA.dispositivos].map((m) =>
      html.indexOf(m),
    )
    expect(ordem.every((i) => i >= 0)).toBe(true)
    // A ordem é a da spec: é ela que o celular lê de cima para baixo.
    expect([...ordem].sort((a, b) => a - b)).toEqual(ordem)
    // Front v2 (Tarefa 7): a grade é a classe de módulo `grade` — os quatro
    // blocos moram DENTRO dela (é ela que vira duas colunas no desktop).
    const grade = html.search(/class="[^"]*_grade_/)
    expect(grade).toBeGreaterThanOrEqual(0)
    expect(grade).toBeLessThan(ordem[0]!)
  })
  ...
  it('GRATIS vê o convite no lugar do botão — mas NÃO perde as preferências', async () => {
    ...
    try {
      ...
      // Front v2 (Tarefa 7): "começa no MVP" virou "são do plano MVP".
      expect(bloco).toContain('são do plano MVP')
    ...
    expect(bloco).toContain('Movimento no Ao Vivo')
    expect(bloco).not.toContain('são do plano MVP')
  })

  // Front v2 (Tarefa 10): o cliente de push deduplica o reenvio da inscrição
  // POR CONTA (`deveEnviarInscricao`, chave `nip.push.ultima-inscricao`), e o
  // `usuarioId` dessa chave vem da SESSÃO DO SERVIDOR, não de estado do
  // cliente. `AtivarAlertas` não renderiza nada no SSR (nasce 'carregando'),
  // então a prova é na fonte: a página passa `d.sessao.usuarioId` ao bloco,
  // e o bloco o repassa ao botão — sem elo solto no meio.
  it('o botão de ativar alertas recebe o usuarioId da sessão, pela página e pelo bloco', async () => {
    const { readFileSync } = await import('node:fs')
    const pagina = readFileSync('src/app/(app)/conta/page.tsx', 'utf8')
    expect(pagina).toMatch(/<BlocoAlertas[^>]*usuarioId=\{d\.sessao\.usuarioId\}/)
    const blocos = readFileSync('src/features/conta/Blocos.tsx', 'utf8')
    expect(blocos).toContain('<AtivarAlertas usuarioId={usuarioId} />')
    const botao = readFileSync('src/features/conta/AtivarAlertas.tsx', 'utf8')
    expect(botao).toContain("from '@/features/pwa/pwa-cliente'")
    expect(botao.match(/inscreverPush\(chavePublicaVapid, usuarioId\)/g) ?? []).toHaveLength(2)
  })
  ...
  it('plano vencido: o bloco diz que o acesso está inativo e mostra o caminho de volta', async () => {
    ...
    try {
      ...
      // O selo fala do ACESSO: com o plano fora de validade, marcar "pago" ao
      // lado do nome do plano diria "confirmado" sobre o que não vale mais.
      // Front v2 (Tarefa 7): o ícone ✓/— virou `data-pago` no selo.
      expect(html.slice(0, html.indexOf(MARCA.conta))).toMatch(/data-pago="false"[^>]*>MENSAL</)
    } finally {
      acessoNoTeste = acessoDeTeste('MVP')
      await banco.db.delete(assinaturas).where(eq(assinaturas.usuarioId, USUARIO_DEMO))
    }
  ...
  it('assinatura sem NOME de plano ainda é uma assinatura: nada de chamada, e o cancelar continua lá', async () => {
    ...
    try {
      ...
      // O selo do topo não pode contradizer o bloco. Front v2 (Tarefa 7): sem
      // nome de plano, o selo mostra o NÍVEL do acesso — a mesma palavra da
      // linha "Plano" do bloco (antes mostrava o status; os dois dizem a
      // mesma coisa sobre quem paga, e nenhum diz "sem plano").
      const topo = html.slice(0, html.indexOf(MARCA.conta))
      expect(topo).not.toMatch(/sem plano/i)
      expect(topo).toMatch(/data-pago="true"[^>]*>MVP</)
    } finally {
      await banco.db.delete(assinaturas).where(eq(assinaturas.usuarioId, USUARIO_DEMO))
    }
  })
  ...
  it('contrato cancelado e sem nome de plano: o selo diz "CANCELADA" e o ícone não dá confirmado', async () => {
    ...
    try {
      ...
      const html = renderToStaticMarkup(await Pagina({ searchParams: Promise.resolve({}) }))
      const topo = html.slice(0, html.indexOf(MARCA.conta))
      // Front v2 (Tarefa 7): sem nome de plano o selo mostra o nível do ACESSO
      // (Grátis) — a mesma palavra da linha "Plano" do bloco —, e não se
      // marca como pago.
      expect(topo).toMatch(/data-pago="false"[^>]*>Grátis</)
      expect(topo).not.toContain('data-pago="true"')
      expect(topo).not.toMatch(/sem plano/i)
  ...
  it('trocar senha e trocar e-mail ficam à vista, nunca dobrados', async () => {
    const { default: Pagina } = await import('@/app/(app)/conta/page')
    const html = renderToStaticMarkup(await Pagina({ searchParams: Promise.resolve({}) }))
    // Em 390px, onde está a maioria de um PWA, duas ações sensíveis da conta
    // não podem perder visibilidade atrás de uma dobra. Front v2 (Tarefa 7):
    // o v2 não dobra nada (nem nome/avatar); o que se trava é o essencial —
    // senha e e-mail fora de qualquer `<details>`.
    const blocoConta = html.slice(html.indexOf(MARCA.conta), html.indexOf(MARCA.assinatura))
    for (const dobra of blocoConta.match(/<details[\s\S]*?<\/details>/g) ?? []) {
      expect(dobra).not.toContain('name="novaSenha"')
      expect(dobra).not.toContain('name="novoEmail"')
    }
    expect(blocoConta).toContain('name="novaSenha"')
    expect(blocoConta).toContain('name="novoEmail"')
  })

  it('a contagem regressiva fala no singular, e para de contar no dia e depois dele', async () => {
    const { proximaCobrancaEmTexto } = await import('@/features/conta/Blocos')
  ...
  it('estatísticas e como funciona saem do corpo e viram o rodapé da tela', async () => {
    ...
    const rodape = html.slice(html.indexOf(MARCA.dispositivos))
...
describe('identidadeDoUsuario — memorizada por requisição, nunca entre requisições', () => {
  it('a exportação é `cache(` do React, com o nome e a assinatura de sempre', async () => {
    // A casca `(app)/layout.tsx` pergunta nome e avatar em TODA tela logada, e
    // `/conta` pergunta de novo na mesma renderização: sem o memo eram duas
    // idas a `usuarios` por navegação à conta. Mesmo padrão de `sessaoAtual`
    // e `avaliarAcesso`: o cache por requisição só existe dentro de uma
    // renderização do React, que o vitest não monta — a forma é travada pela
    // fonte, e o teste abaixo prova que fora dela é chamada direta.
    const { readFileSync } = await import('node:fs')
    const fonte = readFileSync('src/features/conta/carregar.ts', 'utf8')
    expect(fonte).toMatch(/import \{ cache \} from 'react'/)
    expect(fonte).toMatch(/export const identidadeDoUsuario = cache\(async function identidadeDoUsuario\(/)
  })

  it('fora de uma renderização é chamada direta: mudar o banco entre duas chamadas iguais aparece', async () => {
    // O que NÃO pode acontecer é o cache virar memória do processo: o nome
    // trocado em /conta tem que aparecer na requisição seguinte.
    const { usuarios } = await import('@/modules/dominio/db/schema')
    const { identidadeDoUsuario } = await import('@/features/conta/carregar')
    const [antes] = await banco.db.select({ nome: usuarios.nome }).from(usuarios).where(eq(usuarios.id, USUARIO_DEMO))
    try {
      await banco.db.update(usuarios).set({ nome: 'Antes do memo' }).where(eq(usuarios.id, USUARIO_DEMO))
      expect((await identidadeDoUsuario(USUARIO_DEMO, 'demo@teste.com')).nome).toBe('Antes do memo')
      await banco.db.update(usuarios).set({ nome: 'Depois do memo' }).where(eq(usuarios.id, USUARIO_DEMO))
      expect((await identidadeDoUsuario(USUARIO_DEMO, 'demo@teste.com')).nome).toBe('Depois do memo')
    } finally {
      await banco.db.update(usuarios).set({ nome: antes?.nome ?? null }).where(eq(usuarios.id, USUARIO_DEMO))
    }
  })
})
