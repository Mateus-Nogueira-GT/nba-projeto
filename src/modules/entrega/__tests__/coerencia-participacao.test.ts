...
beforeAll(async () => {
  ...
  const [jogador] = await banco.db
    .insert(jogadores)
    .values({
      nomeCompleto: 'Jogador da conferência',
      timeId: casa!.id,
    })
    .returning()
  ...
  const [jogo] = await banco.db
    ...
    .values({
      dataReferencia: DIA,
      dataHoraUtc: new Date(`${DIA}T23:00:00Z`),
      timeCasaId: casa!.id,
...
describe('a mesma participação no perfil, recap e taxa da temporada', () => {
  it.each([
    ...
    { minutos: '0', pontos: 0, rebotes: 0, roubos: 1, estado: 'CONFERIDO', fez: 0 },
    { minutos: '0', pontos: 0, rebotes: 0, bloqueios: 1, estado: 'CONFERIDO', fez: 0 },
    { minutos: '0', pontos: 0, rebotes: 0, cestasT: 1, estado: 'CONFERIDO', fez: 0 },
    { minutos: null, pontos: 0, rebotes: 0, roubos: 1, estado: 'CONFERIDO', fez: 0 },
    { minutos: '0', pontos: 0, rebotes: 0, saldoQuadra: -1, estado: 'CONFERIDO', fez: 0 },
    ...
    await banco.db
      ...
      .set({
        ...
        pontos: caso.pontos,
        rebotesTotal: caso.rebotes,
        assistencias: 0,
        roubos: caso.roubos ?? 0,
        bloqueios: caso.bloqueios ?? 0,
        cestasT: caso.cestasT ?? 0,
        saldoQuadra: caso.saldoQuadra ?? null,
      })
    ...
    expect.soft(perfil).toMatchObject({ estado: caso.estado, fez: caso.fez })
    expect.soft(rodada!.jogadores[0]!.fez).toBe(caso.fez)
    expect.soft(rodada!.conferidos).toBe(conferidos)
    expect.soft(recap).toMatchObject({ conferidos, bateram: acertos })
    expect.soft(taxa).toMatchObject({ conferidos, acertos, rodadas: conferidos })
    expect.soft(ultima).toBe(conferidos === 1 ? DIA : null)
  })
})
